...
- **自动闭环模式** — 触发：`自动循环`、`自动持续`、`直到分数达标`、`闭环完成所有已规划任务`
  - 每次由用户指定目标范围，可为任务列表、父任务下全部子任务或指定的 backlog 集合；对范围内任务依次执行生成 → 本地提交 → 独立评估，低于 8 分时持续修复直至达标
  - 用户明确触发本模式时，允许 AI 创建本地 Git commit；默认不执行 `git push`
