...
2. 读 `docs/backlog.md` + `docs/prd.md` + `docs/tech.md`（及项目维护的 `docs/note.md` 等）获取上下文。**涉及配置/环境问题时，可以或申请读 `.local/` 下的实际配置文件**，不要假设"用户本地配置已就绪"
3. 需求分析：检查依赖项是否满足、判断复杂度（简单/中等/复杂）
4. 产出方案。**无论任务体量，技术方案始终写入 backlog 条目的「方案」字段**（说清涉及哪些模块、大致方向、数据流向）。只有中大体量且需要长期说明的功能设计或架构决策，才额外创建 `docs/design/` 文档作为补充；design 文档不重复 backlog 中已有的技术方案
   - **小体量任务**（单文件改动、纯前端交互调整、简单逻辑修复）：方案仅写入 backlog，不创建 design 文档
   - **中大体量任务**（跨模块、涉及架构决策、需要子阶段拆分，且有需要长期保留的功能/交互/架构决策）：backlog 写技术方案 + 额外产出 design 文档
   - **路径与命名治理**（目录整理、文件迁移/改名、配置键改名及其引用同步）：即使波及多个模块，只要不改变功能、数据模型或用户交互，仍属于小体量任务；仅在 backlog 记录方案和验收，不创建 design 文档
...
5. **必须输出明确的任务列表，且任务拆分的落点是 backlog 条目**，不可将待执行任务埋在大段文字中间，也不可把任务拆分/实施步骤写进 design 文档或中枢文档（后者只做方向性指引与索引）。用户需手动执行的任务以 **（用户）** 前缀着重标记，例如："（用户）在 `.local/` 配置文件中补充对应密钥"
...
- **方案保持高层抽象**：规划阶段描述"要达到什么效果、交互行为如何、大致方向"，不写伪代码、不指定 CSS 属性值、不预设变量名或函数签名。生成模式应有足够的实现自由度，而非单纯执行规划阶段已写好的代码变更
...
3. **归档前评估门槛**：当用户明确表达"准备版本归档，给 backlog 全部 Done 任务做评估"或同义意图时，先进入 `eval-guide.md` 的「归档前阶段合理性专项检查」，完成报告和问题登记后再由用户决定是否继续归档。该检查不适用于普通任务收尾
4. **全 Done 主动提醒**：当任务总览中除明确不执行状态（如 `暂缓`、`已取代`、`已拒绝`）外均为 `Done`，可主动询问用户是否需要执行归档前阶段合理性专项检查；一次全 Done 状态只提醒一次，用户暂缓后等待任务总览变化或用户主动提归档再询问
5. 按任务类型执行：
...
**触发词**：`自动循环` / `自动持续` / `直到分数达标` / `闭环完成所有已规划任务`

> 本模式面向用户明确授权的长程任务。每次的任务范围完全以用户启动时的指定为准，可以是任务列表、某父任务下的全部子任务，或用户指定的任意 backlog 集合；它把生成、提交、评估、修复规划和再次生成串成不等待用户的闭环。

**进入前一次性确认**：按用户指定范围展开的具体任务和顺序、分数门槛、提交是否仅限本地、以及遇到不可自动解决的外部阻塞时的处理方式。范围必须由用户明确提供，AI 不得自行把范围扩展为“全部已规划任务”；其余未明确项默认门槛为 8 分、本地 commit、不 push，记录阻塞并继续下一个任务。

**每个任务的固定循环**：

1. 读取该条目的方案、技术文档、编码规范和最小必要代码，进入 `WIP`，同步任务总览与详情状态。
2. 实现并运行全部可自动执行的验证；提交实现和文档变更。
3. 产出独立评估报告并提交。评分低于门槛时，评估报告必须列明证据与失分点。
4. 为失分点更新 backlog 方案，必要时创建或更新设计文档；提交规划产物。
5. 实现修复、运行验证并提交，再回到第 3 步。
6. 达到门槛后提交复评报告，将任务总览、详情状态和验收勾选同步为 `Done`，再继续下一个任务。

**约束**：

- 本模式仅在用户明确触发时允许 AI 创建本地 Git commit，默认禁止 `git push`。
- 评估必须独立于刚完成的实现审视验收覆盖、测试、回归风险与可维护性，不能只以构建通过代替评分。
- 不可自动解决的外部阻塞写入该任务和评估记录，不得伪造通过；按用户预先确认的策略继续后续任务。
- 自动压缩或新会话恢复时，先读取 backlog、相关设计和最近评估报告，继续未完成循环，不丢失目标范围、门槛和提交策略。
...
1. **评估产出**：评估器将评分报告写入 `../eval/reports/`，如发现需要改进的问题，在 `docs/backlog.md` 中新增条目（状态 = 待规划，只描述问题现象和严重程度，不写方案建议）
