...
<!-- harness:begin src=pancake/30-Tools/harness/common/handbook-eval-guide.md 公共骨架：由 pancake 统一同步，勿在此修改；改动请编辑 pancake 后运行 sync.sh push -->
...
- 需要 tmux、Claude 或 Hermes 真实环境的验证，先检查残留进程和环境，再提供手动验收步骤交由用户执行；不为验证长期遗留后台进程
